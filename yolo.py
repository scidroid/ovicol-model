...
# Create directories
for directory in [
        ORIGINAL_IMAGES_DIR, ORIGINAL_ANNOTATIONS_DIR, TILED_IMAGES_DIR,
        TILED_ANNOTATIONS_DIR, OUTPUT_DIR
]:
...
def fetch_data():
    ...
    if response.status_code == 200:
        ...
        print(
            f"Error: {response.status_code}, Response content: {response.content}"
        )
        return None
...
def yolo_to_absolute(yolo_ann: str, img_width: int, img_height: int):
    ...
    if len(parts) != 5:
        raise ValueError(
            f"Annotation line '{yolo_ann}' does not have 5 parts after splitting. Parts found: {parts}"
        )
    ...
    try:
        ...
        raise ValueError(
            f"Error converting parts of annotation line '{yolo_ann}' to numbers. Original parts: {parts}. Stripped parts: {list(map(str.strip,parts))}. Error: {e}"
        )
...
def create_tiled_dataset(raw_data,
                         tile_size=TILE_SIZE,
                         overlap_ratio=OVERLAP_RATIO,
                         min_visibility_ratio=0.25):
    ...
    original_image_info = {}
    print(
        "Step 1: Downloading original images and collecting annotation data..."
    )
    for image_id, img_data in tqdm(
            raw_data.items(), desc="Downloading and pre-processing images"):
    ...
    print(
        f"Step 1 finished. Downloaded/verified {len(original_image_info)} images. Processed {total_original_annotations_processed} original annotation lines."
    )

    # Now, tile images and adjust annotations
    print("\nStep 2: Creating tiles and adjusting annotations...")
    for image_id, info in tqdm(original_image_info.items(),
                               desc="Creating tiles"):
        ...
        for y in range(0, img_height, step_size):
            for x in range(0, img_width, step_size):
                ...
                if current_tile_height < tile_size or current_tile_width < tile_size:
                    ...
                    tile_img = cv2.copyMakeBorder(tile_img,
                                                  0,
                                                  pad_h,
                                                  0,
                                                  pad_w,
                ...
                tile_annotations = []
                for ann_line_from_api_potentially_dirty in info[
                        "annotations_raw"]:
                    if not isinstance(ann_line_from_api_potentially_dirty,
                                      str):
                        print(
                            f"Warning: non-string annotation found: {ann_line_from_api_potentially_dirty} for image {image_id}. Skipping this annotation."
                        )
                        continue
                    ...
                    for current_yolo_line in potential_yolo_lines:
                        ...
                        try:
                            class_id, orig_xmin, orig_ymin, orig_xmax, orig_ymax = yolo_to_absolute(
                                current_yolo_line, img_width, img_height)
                        except ValueError as e:
                            print(
                                f"Skipping invalid original annotation line: '{current_yolo_line}' for image {image_id}. Details: {e}"
                            )
                            continue

                        obj_box_orig = [
                            orig_xmin, orig_ymin, orig_xmax, orig_ymax
                        ]
                        ...
                        if inter_xmax > inter_xmin and inter_ymax > inter_ymin:
                            ...
                            if obj_orig_area > 0 and (
                                    inter_area /
                                    obj_orig_area) >= min_visibility_ratio:
                                new_xmin_clipped = max(0, new_xmin)
                                new_ymin_clipped = max(0, new_ymin)
                                new_xmax_clipped = min(tile_actual_width - 1,
                                                       new_xmax)
                                new_ymax_clipped = min(tile_actual_height - 1,
                                                       new_ymax)
    ...
    print(
        f"Total original annotations processed: {total_original_annotations_processed}"
    )
    print(
        f"Total annotations created for tiles: {total_tile_annotations_created}"
    )

    if len(raw_data) > len(processed_image_ids):
        print(
            f"Warning: {len(raw_data) - len(processed_image_ids)} original images did not result in any tiles (e.g., no annotations, download error, or no annotations met visibility criteria in tiles)."
        )
...
if not all_tiled_image_paths:
    raise Exception(
        "Failed to create any tiled images. Check logs for errors.")
print(
    f"Successfully created {len(all_tiled_image_paths)} tiled image paths for dataset."
)
...
splits = {
    ...
    'val':
    all_tiled_image_paths[train_split_idx:train_split_idx + val_split_idx],
    'test': all_tiled_image_paths[train_split_idx + val_split_idx:]
}
print(
    f"Dataset split: Train ({len(splits['train'])} tiles), Val ({len(splits['val'])} tiles), Test ({len(splits['test'])} tiles)."
)
...
for split_name, paths in splits.items():
    ...
    for tiled_img_path in tqdm(paths, desc=f"Organizing {split_name} set"):
        ...
        if tiled_annot_path.exists():
            shutil.move(tiled_img_path,
                        new_img_path)  # Changed from copy to move
            shutil.move(tiled_annot_path,
                        new_annot_path)  # Changed from copy to move
        else:
            print(
                f"Warning: Annotation file {tiled_annot_path} not found for image {tiled_img_path}. Skipping this file for {split_name} set."
            )
...
try:
    ...
    results = model.train(
        ...
        degrees=180.0,  # Full rotation augmentation for pill-shaped objects
        scale=0.5,  # Scale augmentation
        mosaic=1.0,  # Enable mosaic augmentation
        mixup=0.3,  # Mixup augmentation
        copy_paste=0.3,  # Copy-paste augmentation
        box=12.0,  # Very high box loss gain to enforce pill shape detection
        cls=0.7,  # Class loss gain
        dfl=2.5,  # Increased DFL loss gain for better boundary detection
        warmup_epochs=WARMUP_EPOCHS,
        warmup_momentum=0.8,
        warmup_bias_lr=0.1,
        close_mosaic=10,
        conf=0.25,  # Confidence threshold during training
        iou=0.7,  # IoU threshold for better box precision
        max_det=100,  # Maximum detections per image
        nms=True,  # Enable Non-Maximum Suppression
        rect=False,  # No rectangular training as objects can be rotated
        perspective=0.0,  # No perspective augmentation to preserve pill shape
        shear=0.0,  # No shear augmentation to preserve pill shape
        hsv_h=0.015,  # Slight hue augmentation
        hsv_s=0.7,  # Increased saturation augmentation
        hsv_v=0.4,  # Increased value augmentation
        fliplr=0.5,  # Enable horizontal flips
        flipud=0.5   # Enable vertical flips since orientation doesn't matter
    )
